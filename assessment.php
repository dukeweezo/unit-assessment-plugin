...
class UnitPlugin
{
    ...
    {
        add_action("init", [$this, "def_assessment_unit_type"]);

        add_filter(
            "manage_assessment-unit_posts_columns",
            [$this, "register_custom_columns"],
            10,
            2
        );
        add_action(
            "manage_assessment-unit_posts_custom_column",
            [$this, "handle_floor_plan_column"],
            10,
            2
        );

        add_action("load-post.php", [$this, "setup_meta_boxes"]);
        add_action("load-post-new.php", [$this, "setup_meta_boxes"]);

        add_action("admin_menu", [$this, "setup_menu"]);

        add_action("wp_ajax_call_from_ui", [$this, "call_from_ui"]);
        add_action("wp_ajax_nopriv_call_from_ui", [$this, "verify_login"]);

        add_shortcode("units", [$this, "def_shortcode"]);
    ...
    public function init_menu()
    {
        ...
        echo '<br><div style="display: inline; padding: .5em; background-color: orange;"><a data-nonce="' .
            $nonce .
            '" href="' .
            $link .
            '">Generate units from API</a></div><br><br>' .
            $success;
    }
    ...
    public function call_from_ui()
    {
        ...
        if ($err) {
            ...
            for ($i = 0; $i < 100; $i++) {
                ...
                update_option(
                    "generate_units_success",
                    "Successfully created 100 units!"
                );
            }
        }
    ...
    public function def_shortcode()
    {
        ...
        $html_equals_one =
            "<h3>Units - area equals 1</h3>" .
            $this->query_and_build_html($query_params_equals_one, get_the_ID());
        $html_greater_than_one =
            "<h3>Units - area > 1</h3>" .
            $this->query_and_build_html(
                $query_params_greater_than_one,
                get_the_ID()
            );
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                if ($outer_id == $id) {
                    ...
                    $html .=
                        '<div class="unit-inner-flex-item"><li>Asset id: ' .
                        $asset_id .
                        "</li></div>";
                    $html .=
                        '<div class="unit-inner-flex-item"><li>Floor id: ' .
                        $floor_id .
                        "</li></div>";
                    $html .=
                        '<div class="unit-inner-flex-item"><li>Building id: ' .
                        $building_id .
                        "</li></div>";
                    $html .=
                        '<div class="unit-inner-flex-item"><li>Floor plan id: ' .
                        $floor_plan_id .
                        "</li></div>";
                    $html .=
                        '<div class="unit-inner-flex-item"><li>Area id: ' .
                        $area_id .
                        "</li></div>";
